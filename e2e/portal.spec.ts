...
test('protected modules have stable desktop visual states', async ({ page }) => {
  await signIn(page)
  const modules = [
...
test('locations, users, logs, and map expose their key state transitions', async ({ page }) => {
  await signIn(page)
  ...
  await expect(page.getByText('Click the map to preview a new position.')).toBeVisible()
})
