...
def _number(value, name, lo=None, hi=None, positive=False):
    ...
    try:
        ...
        raise ValidationError(f"{name} must be a finite number") from None
...
def _enum(value, name, options):
    if not isinstance(value, str) or value not in options:
        raise ValidationError(f"{name} must be one of {', '.join(sorted(options))}")
    return value
...
def _coordinates(data):
    ...
    return (
        _number(data["latitude"], "latitude", -90, 90),
        _number(data["longitude"], "longitude", -180, 180),
    )
...
def _status(value):
    ...
    if value.lower() in {"active", "open"}:
        return "active"
    if value.lower() in {"inactive", "closed"}:
        return "inactive"
    raise ValidationError("status must be active or inactive")
...
def _building(building_id):
    if building_id is not None and not Building.query.get(building_id):
        return "Building not found", 404


def _points(value):
    if not isinstance(value, list):
        raise ValidationError("path_points must be an array")
    result = []
    for sequence_no, raw in enumerate(value, 1):
        if not isinstance(raw, dict):
            raise ValidationError("each path point must be an object")
        lat, lng = _coordinates(raw)
        if _int(raw.get("sequence_no", sequence_no), "sequence_no") != sequence_no:
            raise ValidationError("path_points sequence_no values must be contiguous and start at 1")
        building_id = _int(raw.get("building_id"), "building_id", True)
        if error := _building(building_id):
            return error
        result.append({
...
def _node_values(data, current=None):
    node_type = _enum(
        data.get("node_type", getattr(current, "node_type", "intersection")),
        "node_type",
        NODE_TYPES,
    )
    ...
    for key, lo, hi in (("latitude", -90, 90), ("longitude", -180, 180)):
        if current is None and key not in data:
            raise ValidationError("latitude and longitude are required")
        if key in data:
            values[key] = _number(data[key], key, lo, hi)
    for key in ("location_id", "building_id"):
        values[key] = (
            _int(data[key], key, True)
            if key in data
            else getattr(current, key, None)
        )
    if values["building_id"] is not None and node_type != "entrance":
        raise ValidationError(
            "building_id may only be associated with an entrance route node"
        )
    if error := _building(values["building_id"]):
        return error
    values["name"] = (
        _text(data["name"], "name", True)
        if "name" in data
        else getattr(current, "name", "Route Node")
    )
    values["status"] = (
...
def create_route_node():
    ...
    try:
        ...
        record = RouteNode(**values)
        db.session.add(record)
        db.session.flush()
        log_audit("Admin", None, "create", "Route Node", record.node_id, record.name)
        db.session.commit()
        return jsonify(
            success=True,
            message="Route node created successfully",
            route_node=record.to_dict(),
        ), 201
...
def update_route_node(node_id):
    ...
    try:
        ...
        if not record:
            return _error("Route node not found", 404)
        data = _body()
        values = _node_values(data, record)
        if isinstance(values, tuple):
            return _error(*values)
        for key, value in values.items():
            setattr(record, key, value)
        if "building_id" in data and record.node_type == "entrance":
            action = (
                "associate entrance"
                if record.building_id
                else "clear entrance association"
            )
        else:
            action = "update"
        log_audit("Admin", None, action, "Route Node", node_id, record.name)
        db.session.commit()
        return jsonify(
            success=True,
...
def delete_route_node(node_id):
    ...
    try:
        ...
        log_audit(
            ...
            "Route Node",
            node_id,
            f"{record.name}; connected pathways and path points cascade deleted",
        )
        db.session.commit()
...
def _pathway_values(data, current=None):
    ...
    if values["source_node_id"] == values["destination_node_id"]:
        raise ValidationError(
            "source_node_id and destination_node_id cannot be the same"
        )
    for key in ("source_node_id", "destination_node_id"):
    ...
    values["direction"] = _enum(
        data.get("direction", getattr(current, "direction", "Unknown")),
        "direction",
        DIRECTIONS,
    )
    shade = data.get("shade", getattr(current, "shade", "Unshaded"))
    if "shaded" in data and "shade" not in data:
        if not isinstance(data["shaded"], bool):
            raise ValidationError("shaded must be a boolean")
        shade = "Fully Shaded" if data["shaded"] else "Unshaded"
    values["shade"] = _enum(shade, "shade", SHADES)
    values["surface_type"] = (
        _text(data["surface_type"], "surface_type", False, 50)
        if "surface_type" in data
        else getattr(current, "surface_type", None)
    )
    values["allowed_modes"] = _modes(
        {**data, "path_type": values["path_type"]},
        tuple(x.mode for x in getattr(current, "allowed_modes", [])) or ("Walking",),
    )
    if "path_points" in data:
        values["path_points"] = _points(data["path_points"])
        if isinstance(values["path_points"], tuple):
            return values["path_points"]
    return values


def _apply_pathway(record, values):
    points = values.pop("path_points", None)
    modes = values.pop("allowed_modes")
    for key, value in values.items():
        setattr(record, key, value)
    record.shaded = record.shade in {
        "Fully Shaded",
        "Mostly Shaded",
        "Partial Shade",
    }
...
def create_pathway():
    ...
    try:
        ...
        if points is not None:
            _replace_points(record, points)
        log_audit("Admin", None, "create", "Pathway", record.pathway_id, record.name)
        db.session.commit()
        return jsonify(
            success=True,
            message="Pathway created successfully",
            pathway=record.to_dict(),
        ), 201
    except ValidationError as error:
        return _error(str(error))
    except Exception as error:
...
def update_pathway(pathway_id):
    ...
    try:
        ...
        if points is not None:
            _replace_points(record, points)
        log_audit("Admin", None, "update", "Pathway", pathway_id, record.name)
        db.session.commit()
        return jsonify(
            success=True,
...
def delete_pathway(pathway_id):
    ...
    try:
        ...
        log_audit(
            ...
            "Pathway",
            pathway_id,
            f"{record.name}; path points cascade deleted",
        )
        db.session.commit()
...
def _point_values(data, current=None):
    ...
    values = {
        "pathway_id": _int(
            data.get("pathway_id", getattr(current, "pathway_id", None)),
            "pathway_id",
        ),
        "sequence_no": _int(
            data.get("sequence_no", getattr(current, "sequence_no", None)),
            "sequence_no",
        ),
    }
    if not Pathway.query.get(values["pathway_id"]):
    ...
    values.update({
        "latitude": _number(
            data.get("latitude", getattr(current, "latitude", None)),
            "latitude",
            -90,
            90,
        ),
        "longitude": _number(
            data.get("longitude", getattr(current, "longitude", None)),
            "longitude",
            -180,
            180,
        ),
        "node_type": _enum(
            data.get("node_type", getattr(current, "node_type", None)),
            "node_type",
            PATH_POINT_TYPES,
        ),
        "status": _status(
            data.get("status", getattr(current, "status", "active"))
        ),
    })
    values["building_id"] = _int(
        data.get("building_id", getattr(current, "building_id", None)),
        "building_id",
        True,
    )
    if error := _building(values["building_id"]):
...
def _point_write(point_id=None):
    ...
    try:
        ...
        if point_id is not None and not record:
            return _error("Path point not found", 404)
        values = _point_values(_body(), record)
        if isinstance(values, tuple):
            return _error(*values)
        if record is None:
            record = PathPoint(**values)
            db.session.add(record)
            db.session.flush()
        ...
        log_audit("Admin", None, action, "Path Point", record.point_id, record.node_type)
        db.session.commit()
        return jsonify(
            success=True,
            message=f"Path point {action}d successfully",
            path_point=record.to_dict(),
        ), code
    except ValidationError as error:
        return _error(str(error))
    except Exception as error:
...
def delete_path_point(point_id):
    ...
    try:
        ...
        if not record:
            return _error("Path point not found", 404)
        db.session.delete(record)
        log_audit("Admin", None, "delete", "Path Point", point_id, record.node_type)
        db.session.commit()
