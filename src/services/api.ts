...
export type FailureKey = 'locationSave' | 'routeSave' | 'userUpdate' | 'mapSave'
export const mockFailures: Record<FailureKey, boolean> = { locationSave: false, routeSave: false, userUpdate: false, mapSave: false }
export const setMockFailure = (key: FailureKey, enabled: boolean) => { mockFailures[key] = enabled }
const failIfConfigured = (key: FailureKey) => { if (mockFailures[key]) throw new Error(`Mock ${key} failed.`) }
...
export interface Services {
  ...
  map: { buildings(): Promise<typeof buildings>; locations(): Promise<Location[]>; nodes(): Promise<RouteNode[]>; pathways(): Promise<Pathway[]>; save(): Promise<void> }
...
export const services: Services = {
  ...
  locations: { list: async q => wait({ items: clone(q ? locations.filter(l => [l.name,l.code,l.type,l.function ?? '',l.keywords ?? ''].some(v => matches(v,q))) : locations), total: locations.length, page: 1, pageSize: 20 }), save: async location => { failIfConfigured('locationSave'); const i = locations.findIndex(l => l.id === location.id); if (i >= 0) locations[i] = clone(location); else locations.push(clone(location)); addAudit('Updated Location', location.name); return wait(clone(location)) }, remove: async id => { const i = locations.findIndex(l => l.id === id); if (i >= 0) addAudit('Removed Location', locations.splice(i, 1)[0].name); return wait(undefined) } },
  routes: { list: async q => wait({ items: clone(q ? pathways.filter(p => matches(p.name,q)) : pathways), total: pathways.length, page: 1, pageSize: 20 }), save: async path => { failIfConfigured('routeSave'); const i = pathways.findIndex(p => p.id === path.id); if (i >= 0) pathways[i] = clone(path); else pathways.push(clone(path)); addAudit('Updated Route', path.name); return wait(clone(path)) }, remove: async id => { const i = pathways.findIndex(p => p.id === id); if (i >= 0) addAudit('Removed Route', pathways.splice(i, 1)[0].name); return wait(undefined) } },
  users: { list: async q => wait({ items: clone(q ? users.filter(u => matches(u.username,q)) : users), total: users.length, page: 1, pageSize: 20 }), update: async user => { failIfConfigured('userUpdate'); const i = users.findIndex(u => u.id === user.id); if (i >= 0) users[i] = clone(user); addAudit('Updated User', user.username); return wait(clone(user)) }, remove: async id => { const i = users.findIndex(u => u.id === id); if (i >= 0) addAudit('Removed User', users.splice(i, 1)[0].username); return wait(undefined) } },
  ...
  map: { buildings: async () => wait(clone(buildings)), locations: async () => wait(clone(locations)), nodes: async () => wait(clone(routeNodes)), pathways: async () => wait(clone(pathways)), save: async () => { failIfConfigured('mapSave'); addAudit('Updated Map', 'Campus geometry'); return wait(undefined) } },
  imports: {
