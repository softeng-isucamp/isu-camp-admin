...
export function Logs() {
 const [q,setQ]=useState(""),[category,setCategory]=useState("all"),[date,setDate]=useState("all"),[page,setPage]=useState(1),[detail,setDetail]=useState<AuditEntry|null>(null); const pageSize=20;
 const {data}=useQuery({queryKey:["logs",category,q,date,page],queryFn:()=>services.logs.list(category==="all"?"All":category,q,"All Actors",date,page,pageSize)});
 useEffect(()=>setPage(1),[q,category,date]); const result=data??{items:[],total:0,page:1,pageSize};
